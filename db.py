...
def get_expiring_keys():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor.execute(
            """
            SELECT keys.id, keys.user_id, keys.email, keys.expiry_time, users.tg_id, keys.notified_level, keys.inbound_id
            FROM keys
        ...
        for row in cursor.fetchall():
            key_id, user_id, email, expiry, tg_id, notified_level, inbound_id = row
...
def use_all_bonuses(user_id: int) -> int:
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()
        now = int(time.time())
        cursor.execute(
            """
            SELECT id, days FROM bonuses
            WHERE user_id = ? AND status = 'active' AND expiry_time > ?
            ORDER BY expiry_time
            """,
            (user_id, now),
