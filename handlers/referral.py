...
async def choose_bonus_key(update: Update, context: CallbackContext):
    ...
    if not keys:
        ...
        return
    keyboard = []
    for key_id, email, expiry, active, _ in keys:
...
async def apply_bonus_button(update: Update, context: CallbackContext):
    ...
    if inbound_id == 2:
        await query.edit_message_text(
            "⛔️ Этот ключ больше не поддерживает бонусное продление.\n\n"
            "⚙️ Пожалуйста, перенесите его на новую систему — это бесплатно и займёт пару секунд.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📦 Перенести ключ", callback_data=f"migrate_{key_id}")],
...
async def referral_menu(update: Update, context: CallbackContext):
    ...
    text = (
        ...
        "• 🎉 +3 дня — за регистрацию друга вам и вашему другу.\n"
        "• 💸 +20% бонуса от его каждой покупки (например, покупка на 30\u202fдн → ты получаешь 6\u202fдн, на 90\u202f → 18\u202fдн)\n"
        "• 🏅 +10 дней — за 3 платящих рефералов\n"
        "• 🥇 +15 дней — за 5 платящих рефералов\n\n"
        f"🔗 *Ваша ссылка:* `{link}`\n\n"
